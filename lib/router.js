...
Router.prototype.findDAOFactory = function(modelName) {
  return this.sequelize[modelName];
}
    ...
    daoFactory
      ...
      .then(function(entry) {
        if (!entry){
          ...
          this.handleSuccess(entry.dataValues, callback)
        }
      }.bind(this))
      .error(function(err) {
        this.handleError(err, callback)
          ...
          entry[association.accessors.get]()
            .then(function(entries){
              ...
              if (Array.isArray(entries)) {
                entries = entries.map(function(entry) { return entry.dataValues })
              } else {
                entries = entries.dataValues
              }
